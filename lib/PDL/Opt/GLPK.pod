...
The coeffient matrix C<a> must be 2-d, though broadcasting over higher
dimensions is possible for other arguments.
See L</glpk_bc> for an alternative.
...
This is a wrapper for C<glpk> that takes the same arguments, but has two
differences:
...
The coefficient matrix C<a> must be a true ndarray and cannot be a
L<PDL::CCS::Nd> sparse matrix.
